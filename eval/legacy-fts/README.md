...
Legacy FTS retrieval rows use the same fair metric columns as the graph
backends: effective recall ignores questions without ground-truth sources,
while answer scoring separates raw entity mentions from grounded entity hits so
refusal text does not receive entity-match credit. Evaluator subprocesses call
the unified legacy surface directly with `kb ask --engine legacy` and
`kb find --engine legacy`; old `kb legacy` invocations should not appear in new
captures.
