...
The project has moved from scaffold+implementation through the GraphRAG pivot into a mostly stable GraphRAG-first system. The remaining work is evaluation, final reporting, and carefully bounded polish rather than another architecture rebuild.
...
7. `kb ask --method auto|basic|local|global|drift` exposes GraphRAG query modes through the default answer controller with deterministic auto-routing.
...
In short, the core workflow works end-to-end, and the pivot keeps that work instead of discarding it. The project is now GraphRAG-first while preserving the original wiki system as the artifact layer and exposing old FTS retrieval only through explicit legacy commands.
