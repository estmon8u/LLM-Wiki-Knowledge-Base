...
Updated: 05/20/2026 to fold the custom WikiGraphRAG backend into `kb update`, `kb find`, and `kb ask` for backend comparison while keeping the existing [Command Line Interface Guidelines](https://clig.dev/) (clig.dev) requirements such as consistent flags, discoverable help, machine-readable `--json` output, sensible defaults, and friendly errors
...
A research-grounded RAG evaluator (`scripts/evaluate_rag.py` / `scripts.rag_eval`) compares legacy FTS, Microsoft GraphRAG, WikiGraphRAG classic, and WikiGraphRAG LightRAG on the benchmark, retrieval-only by default and provider-backed answers, RAGAS, and LLM-judge metrics opt-in via `--allow-provider-calls`, `--ragas`, and `--judge`. It writes raw rows, bootstrap summaries, and leaderboards under `eval/rag_eval/`; `scripts/evaluate_backends.py` remains as a compatibility wrapper for older `--backends ...` invocations.

## May 20 2026 WikiGraphRAG Quality Hardening + 10-PDF Three-way Evaluation

After integrating WikiGraphRAG as a peer to GraphRAG and Legacy FTS, the first backend evaluation on the 10-PDF corpus showed the gap between WikiGraphRAG and Legacy FTS was real but the early metric definitions were still gameable. The entity-hits metric was near-tied because refusals that name-dropped the missing entity got the same credit as grounded answers. A targeted hardening pass produced the following layered improvements:

### 1. Fairer evaluator metrics

The previous metrics over-credited refusals and dragged averages down on intentionally-empty questions. The current evaluator (`scripts/rag_eval/`, launched by `scripts/evaluate_rag.py`) reports:
