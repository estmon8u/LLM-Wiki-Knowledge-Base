...
Date: 04/18/2026
Updated: 05/11/2026 for GraphRAG pivot Phase 5
...
| Project Update 2: GraphRAG pivot and input bridge | Apr 29 to May 12 | Isolate legacy FTS behavior, add the Microsoft GraphRAG dependency and workspace, sync normalized artifacts into GraphRAG JSON input, and prepare the baseline comparison workflow | Update 2 shows the controlled GraphRAG pivot and `kb graph sync` |
| Project Update 3: graph indexing, query modes, and maintenance validation | May 13 to May 26 | Wrap GraphRAG indexing/query behavior, evaluate local/global/basic/DRIFT modes on the real corpus, run stale-page and graph-input freshness checks, and measure refresh behavior | Update 3 demonstrates GraphRAG querying plus synthesis and maintenance validation |
