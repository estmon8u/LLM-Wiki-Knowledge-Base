...
Date: 04/18/2026
Updated: 05/11/2026 for GraphRAG pivot Phase 3
...
This adjustment preserves the Karpathy-style wiki idea as the artifact and maintenance layer while moving retrieval and synthesis to GraphRAG. The existing SQLite FTS5/wiki retrieval path has been demoted to explicit deprecated `kb legacy find` and `kb legacy ask` commands, and should be compared against GraphRAG Basic, Local, Global, and DRIFT search only as historical evidence for the pivot. Phase 3 has added the Microsoft GraphRAG dependency, initialized the project-local GraphRAG workspace under `graph/graphrag/`, and implemented `kb graph sync` so normalized inputs and manifest metadata become GraphRAG JSON input. The remaining work is to build the index and expose graph query modes through the CLI.
...
The expected workflow is as follows. First, `kb add` will accept either a single file or a directory, recurse through directory inputs by default, detect source type, convert each supported document into canonical markdown or plain text, store the original files in `raw/`, store the normalized representations, update the manifest, and record searchable metadata plus a source hash. Second, `kb update` will add any provided source paths, then compile or refresh source pages, regenerate the wiki index, generate concept pages, refresh legacy index artifacts, append to the activity log, and persist compile-run state so the most recent failed or interrupted compile can be resumed. Generated wiki artifacts, manifest updates, and vault-export files are written atomically so partial writes are less likely if a run is interrupted. In the current implementation, source pages keep deterministic provenance fields and excerpts from the normalized text while the summary field is generated by the configured provider. Concept pages are generated during update from compiled source pages rather than directly from raw files, and those synthesized claims must retain citations back to the source pages used. Third, `kb graph sync` writes normalized artifacts and manifest metadata into `graph/graphrag/input/sources.json`, configures GraphRAG JSON input, and prepends provenance metadata into chunks for later indexing. The next GraphRAG step is to build the graph index and make GraphRAG the default path for `kb ask` and graph query modes. The current SQLite FTS5 retrieval path is retained only under explicit deprecated `kb legacy find` and `kb legacy ask` commands; it is not an implicit fallback when GraphRAG is missing or stale. Use `kb legacy ask --save` or `--save-as` while GraphRAG querying is still being built to persist useful answers as analysis pages so that query results compound into the maintained knowledge base. Fourth, `kb lint` will detect broken links, orphan pages, missing citations, stale pages whose sources changed, graph-input freshness gaps, or structural gaps that require review. Fifth, `kb review` will run semantic checks across compiled pages, surfacing potential contradictions and terminology drift; it combines deterministic overlap checks with a single provider-backed JSON review pass. Sixth, `kb status` will summarize corpus size, compile state, conversion state, graph state, and maintenance findings; `kb status --changed` shows a pre-compile preview of which sources are new, changed, or already up to date. Finally, `kb export` will write an Obsidian-friendly vault view of the maintained knowledge base.
