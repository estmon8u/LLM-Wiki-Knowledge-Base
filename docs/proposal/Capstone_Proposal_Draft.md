...
Date: 04/18/2026
Updated: 05/11/2026 for GraphRAG pivot Phase 6
...
This adjustment preserves the Karpathy-style wiki idea as the artifact and maintenance layer while moving retrieval and synthesis to GraphRAG. The existing SQLite FTS5/wiki retrieval path has been demoted to explicit deprecated `kb legacy find` and `kb legacy ask` commands, and should be compared against GraphRAG Basic, Local, Global, and DRIFT search only as historical evidence for the pivot. Through Phase 6, the project has added the Microsoft GraphRAG dependency, initialized the project-local GraphRAG workspace under `graph/graphrag/`, implemented `kb graph sync` so normalized inputs and manifest metadata become GraphRAG JSON input, wrapped `kb graph init`, `kb graph index`, and `kb graph status` around the official GraphRAG CLI, exposed `kb graph ask --method local|global|drift|basic` for explicit graph queries, and added `kb graph export-wiki` to convert GraphRAG output tables into generated markdown under `wiki/graph/`. The remaining work is to route the default `kb ask` behavior through a GraphRAG-aware controller and expand graph freshness checks.
