...
| `find` | `src/graphwiki_kb/commands/find.py` | `src/graphwiki_kb/services/graphrag_find_service.py` for direct graph artifacts, `src/graphwiki_kb/services/search_service.py` for maintained-wiki navigation or `--engine legacy` source-page FTS, and `src/graphwiki_kb/services/wikigraph_query_service.py` for WikiGraphRAG contexts |
| `ask` | `src/graphwiki_kb/commands/ask.py` | `src/graphwiki_kb/services/wikigraph_query_service.py` by default, `src/graphwiki_kb/services/graph_ask_controller_service.py` plus `src/graphwiki_kb/services/query_router_service.py` for `--engine graphrag`, and `src/graphwiki_kb/services/query_service.py` for `--engine legacy` |
...
| Legacy search | compiled wiki artifacts | deprecated `kb find --engine legacy` comparator matches from source pages only, preserving the old FTS behavior without a standalone `kb legacy` command group |
...
| Export | compiled wiki plus display assets | Obsidian-friendly vault view; common image assets are copied with markdown, and clean mode removes only markdown paths absent from the current export set |
