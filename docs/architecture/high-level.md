...
5. Search the maintained wiki index through top-level `kb find`, and ask GraphRAG-backed questions by default; access the old FTS path only through explicit `kb legacy ...` commands.
...
8. Lint the maintained knowledge base for broken structure or stale content (deterministic), including GraphRAG input/index/export staleness.
...
- Search storage currently persists a rebuildable SQLite FTS5 chunk index at `graph/exports/search_index.sqlite3` so lexical retrieval no longer scans every markdown file on each query. Top-level `kb find` uses the maintained wiki search index for non-generative navigation across source, concept, analysis, and generated graph pages. In the GraphRAG pivot, FTS-backed answer behavior is temporary legacy infrastructure, not a peer default, and is reachable only through explicit `kb legacy find` / `kb legacy ask` commands with deprecation warnings. Top-level `kb ask` fails with clear next-step guidance when the GraphRAG workspace or index is missing instead of silently falling back to FTS5.
- The GraphRAG workspace is initialized under `graph/graphrag/` through the installed GraphRAG CLI, with tracked settings, prompts, and input scaffolding. `kb init` creates the workspace and syncs managed `kb.config.yaml` graph provider/model/embedding/API-key settings into `graph/graphrag/settings.yaml` while preserving unrelated user-owned GraphRAG tuning; bundled prompts are refreshed when repository templates change. `kb update` plans settings/input sync without mutating workspace files during preflight, applies the planned input when indexing can proceed, records source/runtime digests and active output metadata, then auto-selects full `fast`, incremental `fast-update`, retry after a failed latest run, or skip; `kb status` reports settings, input, the active output directory from the latest successful complete run, output tables, and last recorded index-run metadata; `kb ask --method auto|basic|local|global|drift` is the user-facing GraphRAG answer controller with deterministic mode routing and conservative support-level metadata; and `kb export` turns complete active GraphRAG output tables into inspectable markdown under `wiki/graph/` when output exists. Local `.env`, generated input, output, cache, logs, and graph run metadata are ignored. The old `kb graph` command group has been removed.
