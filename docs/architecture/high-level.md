...
- `graph/` stores the SQLite FTS5 search index and compile-run state under `graph/exports/`, plus the initialized Microsoft GraphRAG workspace under `graph/graphrag/`.
...
- The GraphRAG workspace is initialized under `graph/graphrag/` with tracked settings, prompts, and input scaffolding. The next layer will sync normalized source artifacts and manifest metadata into `graph/graphrag/input/`, delegate graph indexing and query modes to Microsoft GraphRAG, and export selected graph artifacts back into the wiki layer for inspection. Local `.env`, output, cache, and log artifacts are ignored.
