...
5. Ask GraphRAG-backed questions by default; access the old FTS path only through explicit `kb legacy ...` commands.
...
- The GraphRAG workspace is initialized under `graph/graphrag/` through the installed GraphRAG CLI, with tracked settings, prompts, and input scaffolding. `kb init` creates the workspace and syncs `kb.config.yaml` graph provider/model/embedding/API-key settings into `graph/graphrag/settings.yaml`; `kb update` syncs normalized source artifacts and manifest metadata into `graph/graphrag/input/sources.json`, records source/runtime digests and active output metadata, then auto-selects full `fast`, incremental `fast-update`, retry after a failed latest run, or skip; `kb status` reports settings, input, active output directory, output tables, and last recorded index-run metadata; `kb ask --method auto|basic|local|global|drift` is the user-facing GraphRAG answer controller with deterministic mode routing and conservative support-level metadata; and `kb export` turns complete active GraphRAG output tables into inspectable markdown under `wiki/graph/` when output exists. Local `.env`, generated input, output, cache, logs, and graph run metadata are ignored. The old `kb graph` command group has been removed.
