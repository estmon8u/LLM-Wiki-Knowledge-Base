...
4. Sync normalized artifacts into the initialized GraphRAG workspace with source metadata and automatically refresh or skip the graph index based on source/runtime changes.
5. Build graph outputs for graph-first retrieval modes when sync selects a full or incremental GraphRAG index job.
