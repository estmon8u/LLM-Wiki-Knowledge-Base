...
| `eval/benchmark.yaml` | Benchmark questions and expected methods/sources for deprecated FTS, Microsoft GraphRAG, and WikiGraphRAG comparison; includes expected answer terms, forbidden answer terms, insufficient-evidence inference from expected behaviors, unified `--engine` command metadata, and backend-specific expected methods |
...
| `scripts/backend_evaluation_lib.py` | Three-way backend evaluator for legacy, Microsoft GraphRAG, and WikiGraphRAG; subprocess GraphRAG and legacy answer rows pass explicit `--engine` selectors, thread `claim_support`, `graph_data_references`, and source-trace data into scoring, and route GraphRAG retrieval through parquet `text_units` / `community_reports` by default for the apples-to-apples comparison |
