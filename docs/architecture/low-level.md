...
| `src/graphwiki_kb/providers/retry.py` | Shared Tenacity retry decorator (`provider_retry()`) for all `generate()` calls: 3 attempts, exponential backoff with jitter, retryable-status-code-aware transient retry |
...
| `src/graphwiki_kb/providers/anthropic_provider.py` | Anthropic messages provider; detects current Claude adaptive-thinking model identifiers by version pattern, sends the adaptive-thinking flag plus effort, uses `output_config.format` for schema-bound requests, keeps legacy manual thinking budgets for older models, and applies `@provider_retry()` on `generate()` |
| `src/graphwiki_kb/providers/gemini_provider.py` | Google Gemini provider with stable `gemini-2.5-flash` default; `@provider_retry()` on `generate()`, model-sensitive thinking budget/level configuration, and recursive JSON-schema subset normalization with transformation reporting when unsupported keywords such as `additionalProperties` are stripped |
...
| `src/graphwiki_kb/commands/ask.py` | GraphRAG/WikiGraphRAG answer entry point with TTY-aware query status rendering, visible route reasons, `--show-source-trace` output for seed entities, communities, sub-questions, context traces, and provider status; rejects deprecated source-evidence `--limit` |
...
| `src/graphwiki_kb/commands/status.py` | Status command with GraphRAG health summary, JSON graph details, and `--strict` readiness exit-code contract; `--changed` for pre-update diff view |
| `src/graphwiki_kb/commands/export_cmd.py` | Vault export command that refreshes graph wiki pages when output exists; `--wikigraph-modes current|classic|lightrag|all` can refresh WikiGraphRAG cards before copying the vault; `--clean` removes only vault markdown files absent from the current export set |
...
| `src/graphwiki_kb/services/config_service.py` | Config loading, Pydantic-backed provider/conversion/graph validation with unknown top-level keys forbidden outside `extensions`, schema defaults, `schema_excerpt()` helper for extracting schema sections by heading, and lock-protected migration of legacy `kb.config.yaml` versions only after the migrated payload validates |
...
| `src/graphwiki_kb/services/graphrag_workspace_service.py` | Prepares the project-local `graph/graphrag/` workspace, delegates reproducible non-interactive initialization to the GraphRAG Python-entrypoint adapter, renders managed settings without writing during planning, syncs `kb.config.yaml` graph settings into GraphRAG `settings.yaml` while preserving user-owned tuning, copies bundled prompt templates only when missing, and writes changed bundled templates as sibling `*.new` files |
| `src/graphwiki_kb/services/graphrag_command_service.py` | Adapter over GraphRAG's installed Python initialization, indexing, update, and query entrypoints, with signature-aware kwargs, known additive defaults for GraphRAG 3.0.x, unsupported-kwarg filtering, returned-answer capture for non-streaming queries, documented-CLI fallback for entrypoint contract drift, workspace locking, and structured run-result metadata |
...
| `src/graphwiki_kb/services/graphrag_freshness_service.py` | Shared streaming digest helpers and GraphRAG freshness comparison for current input, source hashes, settings, prompts, GraphRAG runtime identity, and last successful index-run metadata |
| `src/graphwiki_kb/services/graphrag_status_service.py` | Reports GraphRAG workspace readiness, synced input counts, active output directory preferring the latest successful complete recorded run, digest-based freshness state, method-specific query artifact readiness, normalized graph state, table health states (`missing`, `dependency_missing`, `present_unreadable`, or `ready`), vector-store state (`missing`, `empty`, `ready`, `unreadable`, or `incompatible`), corrupt run metadata, wiki export presence, row counts, and lock-protected ignored local index-run metadata |
| `src/graphwiki_kb/services/graphrag_sync_service.py` | Coordinates GraphRAG sync/index decisions used by `kb update`: plans workspace settings/input without preflight side effects, applies settings/input under the workspace lock when graph work can proceed, compares source/runtime digests including GraphRAG package and managed schema versions to the last successful index, chooses full rebuild, incremental update, retry after a failed latest attempt, or skip, and records reproducibility metadata |
| `src/graphwiki_kb/services/graphrag_query_service.py` | Requires a method-ready fresh graph index, runs explicit GraphRAG query modes, separates parsed answer text from raw stdout/stderr diagnostics, computes both the synced GraphRAG input hash and recorded raw manifest hash when available, refuses blank saves, and saves optional unique GraphRAG analysis pages with lint-compatible analysis frontmatter, parsed `[Data: ...]` references when available, source trace, separate raw stdout/stderr audit sections, and lock-protected wiki log entries |
| `src/graphwiki_kb/services/graphrag_find_service.py` | Searches active GraphRAG entity and relationship Parquet artifacts directly for top-level `kb find` through streaming PyArrow batch scans with bounded top-result retention and token-aware scoring that avoids short-term substring false positives; command-level merging globally ranks graph and wiki candidates after dedupe |
...
| `src/graphwiki_kb/services/graphrag_wiki_export_service.py` | Reads active GraphRAG Parquet output tables through PyArrow batches under the workspace lock and generates marked `wiki/graph/` markdown pages for documents, text units, entities, relationships, communities, and the graph index with index-run/runtime-digest metadata; preserves manual graph notes, fences raw document/text-unit content, escapes table cells, and caps high-volume relationship page export |
| `src/graphwiki_kb/services/graphrag_input_sync_service.py` | Plans or syncs manifest-backed normalized artifacts into compact `graph/graphrag/input/sources.json`, reports payload size warnings, validates the GraphRAG workspace settings file, configures JSON input columns, lists provenance metadata fields for chunk prepending, and can report skipped missing normalized artifacts for tolerant normal updates while keeping strict graph-only behavior |
| `src/graphwiki_kb/services/normalization_service.py` | Document-type normalization routing for direct text inputs, Mistral OCR-backed native documents and images with a 100 MB inline payload preflight guard, PDF's ordered local fallback chain after Mistral (`Docling` then `MarkItDown` by default), HTML OCR rendered through `wkhtmltopdf` first with local-file access disabled by default and `xhtml2pdf` as fallback, MarkItDown-backed born-digital converters, explicit fallback metadata, and conversion quality gates |
| `src/graphwiki_kb/services/ingest_service.py` | Raw-source copy, normalized-artifact write, streaming origin-hash duplicate short-circuiting before expensive normalization, content duplicate detection after normalization, source registration, deterministic recursive directory ingest with project-managed/generated directories skipped, and callback-friendly batch progress hooks used by `kb add` |
...
| `src/graphwiki_kb/services/export_service.py` | Vault export generation with atomic copies of wiki markdown plus common display assets into the Obsidian view and `--clean` deletion based on the current run's exact exported markdown-path set |
| `src/graphwiki_kb/services/status_service.py` | Project, corpus, and GraphRAG status reporting |
| `src/graphwiki_kb/services/update_service.py` | Orchestrates the full update workflow: preflight → ingest → compile → concepts → search refresh → GraphRAG sync/index/export, plus explicit graph method validation, graph-only maintenance, normal-update missing credential warnings, tolerant missing-normalized graph input warnings, search fallback warnings, export-on-skip for complete graph output, and explicit allow-partial graph failure handling; resolves WikiGraphRAG build/export tri-state flags from authoritative config when the CLI or agent leaves them unset |
| `src/graphwiki_kb/services/wikigraph_index_service.py` | Service wrapper for the custom WikiGraphRAG index build; reads manifest records, resolves normalized source TextUnit settings, writes JSON graph, lexical, community, document, text-unit, chunk, and run metadata artifacts under `graph/wikigraph/`, and can export generated entity/community/chunk/TextUnit cards under `wiki/wikigraph/` or a mode-separated subdirectory such as `wiki/wikigraph/classic/`; generated card filenames are length-capped with deterministic hash suffixes and old generated cards in the target artifact folders are cleared before rewrite |
| `src/graphwiki_kb/services/wikigraph_query_service.py` | Service wrapper for provider-free and provider-backed WikiGraphRAG find/ask operations over the custom graph context builder; threads runtime retrieval settings such as `max_context_tokens` and `lexical_backend` into query-time context assembly |
| `src/graphwiki_kb/wikigraph/answer_service.py` | Provider-free and provider-backed WikiGraphRAG answer synthesis; normalizes provider-emitted citations to the canonical retrieved ref when the provider cites a neighboring TextUnit on the same source path, preserving grounding without falsely marking the answer insufficient |
| `src/graphwiki_kb/wikigraph/context_builder.py` | Builds WikiGraphRAG basic, local, global, and drift-lite contexts; enforces `max_context_tokens` across all methods, records a `budget:<used>/<budget>-tokens` trace when ranked contexts are trimmed, retrieves TextUnits and entity -> document -> TextUnit local-search hops, supports word-boundary/acronym entity matching, applies the conservative section-title boost plus drift-lite additive cross-bundle bonus, and keeps RRF/alias-expansion helpers available for controlled ablations |
| `src/graphwiki_kb/wikigraph/entity_extractor.py` | Deterministic WikiGraphRAG entity and claim extraction from wiki pages; lifts the prefix before `:` in page titles into aliases so titles such as `REALM: Retrieval-Augmented Language Model Pre-Training` match user questions that say `REALM` |
| `src/graphwiki_kb/wikigraph/query_service.py` | Provider-free WikiGraphRAG query engine; `method="auto"` is intent-aware, routing comparison prompts to `drift-lite`, corpus/theme prompts to `global`, entity matches to `local`, and unmatched prompts to `basic` |
| `src/graphwiki_kb/wikigraph/source_text_units.py` | Deterministic, provider-free builder for source-derived TextUnits from `raw/normalized/` or configured text raw fallback paths; records source id, slug, title, origin, hashes, unit index, character spans, and stable text-unit IDs without reading binary source formats directly |
| `src/graphwiki_kb/wikigraph/deps.py` | Lazy optional dependency guard for WikiGraphRAG packages; base installs can run unrelated commands, while graph build/query paths require the `wikigraph` extra before importing NetworkX-backed code |
...
| `src/graphwiki_kb/agents/tool_registry.py` | Builds SDK function tools and direct callable wrappers for read tools plus approval-gated `ingest_recommendation` and `update_kb` write tools; the wrappers stay synchronous because GraphRAG query/index entrypoints set signal handlers on the main thread |
...
| `graph/wikigraph/` | Generated by WikiGraphRAG indexing; ignored JSON graph, lexical, community, source-document, text-unit, LightRAG source-chunk/entity/relation/vector, and run metadata artifacts that can be rebuilt from project wiki and normalized source state |
...
| `wiki/wikigraph/` | Optional generated WikiGraphRAG artifact layer from `kb update --export-wikigraph-artifacts` or `kb export --wikigraph-modes`; pages carry generated frontmatter and are excluded from the next WikiGraphRAG build so exported entities, communities, chunks, relations, source chunks, and TextUnits do not recursively become source evidence; classic and LightRAG generated filenames are kept Windows-vault friendly and LightRAG relation cards include stable relation-id suffixes to avoid overwrites |
| `eval/benchmark.yaml` | Benchmark questions and expected methods/sources for deprecated FTS, Microsoft GraphRAG, WikiGraphRAG classic, and WikiGraphRAG LightRAG comparison; includes expected answer terms, forbidden answer terms, insufficient-evidence inference from expected behaviors, unified `--engine` command metadata, and backend-specific expected methods including `wikigraph-classic` / `wikigraph-lightrag` |
| `eval/rag_eval/` | Current RAG evaluation report directory for raw rows, bootstrap summaries, leaderboard markdown, optional RAGAS/judge rows, and per-run artifacts that may contain local corpus text or model output |
| `scripts/rag_eval/` | Current four-backend evaluation harness for legacy FTS, Microsoft GraphRAG, WikiGraphRAG classic, and WikiGraphRAG LightRAG; loads benchmark schemas, invokes CLI backends, computes rank-aware retrieval metrics, optional provider/RAGAS/judge answer metrics, bootstrap aggregates, and leaderboard outputs |
| `scripts/graphrag_artifact_retriever.py` | BM25-style evaluator retriever over GraphRAG parquet outputs (`text_units`, `community_reports`, `entities`, `relationships`) with `text_units`, `artifact`, and `both` modes; supports fairer comparison than entity-only artifact scans |
| `scripts/fetch_eval_corpus.py` | Downloads the fixed 10-paper arXiv RAG corpus used by the real WikiGraphRAG vs Microsoft GraphRAG verification run for reproducible corpus setup |
| `scripts/evaluate_rag.py` | Research-oriented RAG evaluator for `direct`, legacy, Microsoft GraphRAG, `wikigraph-classic`, and `wikigraph-lightrag`, with retrieval-only local-safe runs plus opt-in provider answer, RAGAS, and LLM-judge scoring |
| `scripts/evaluate_backends.py` | Compatibility wrapper that preserves older `--backends` invocations while delegating to `scripts/evaluate_rag.py` / `scripts.rag_eval` |
