...
| `src/graphwiki_kb/commands/ask.py` | GraphRAG-aware default answer entry point with TTY-aware query status rendering, visible route reasons, and `--show-source-trace`; rejects deprecated source-evidence `--limit`, delegates to the graph ask controller, and never routes to FTS5 |
