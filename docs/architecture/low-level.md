...
| `graph/graphrag/settings.yaml` | Initialized Microsoft GraphRAG workspace settings; currently uses `GRAPHRAG_API_KEY`, `gpt-4.1-mini`, and `text-embedding-3-small` for lower-cost early graph experiments |
...
| `graph/graphrag/input/.gitkeep` | Tracked input-directory scaffold for the upcoming normalized-corpus sync phase |
