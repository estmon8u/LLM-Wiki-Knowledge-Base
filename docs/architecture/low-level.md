...
| `src/graphwiki_kb/services/wikigraph_index_service.py` | Service wrapper for the custom WikiGraphRAG index build; reads manifest records, resolves normalized source TextUnit settings, writes JSON graph, lexical, community, document, text-unit, chunk, and run metadata artifacts under `graph/wikigraph/`, and can export generated entity/community/chunk/TextUnit cards under `wiki/wikigraph/` |
