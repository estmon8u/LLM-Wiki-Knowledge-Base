...
| `src/graphwiki_kb/services/graphrag_find_service.py` | Searches active GraphRAG entity and relationship Parquet artifacts directly for top-level `kb find` through streaming PyArrow batch scans with bounded top-result retention; command-level merging globally ranks graph and wiki candidates after dedupe |
...
| `.github/workflows/tests.yml` | CI matrix for Python 3.11, 3.12, and 3.13 on `main`, `GraphRAG`, and pull requests with Poetry install, Black, Ruff, bounded mypy, CLI smoke checks, pytest, and coverage artifact upload |
