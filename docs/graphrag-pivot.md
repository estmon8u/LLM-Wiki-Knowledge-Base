...
- Phase 9 local workspace source of truth: `pyproject.toml` declares `graphrag`; `kb.config.yaml` version 7 owns the `graph` provider/model/embedding defaults, graph routing aliases, OpenAI API backend selection, and trusted HTML local-file access flag while API-key environment variables are resolved from `providers` unless graph-specific overrides are set; `kb init` creates and syncs `graph/graphrag/settings.yaml` through a signature-aware Python-entrypoint adapter; bundled prompt templates are wheel-included and discovered from packaged or editable layouts; `kb update` writes `graph/graphrag/input/sources.json` from `raw/_manifest.json` and `raw/normalized/`, auto-selects full/update/skip index actions from input digests, source hashes, and GraphRAG package/settings/prompt/schema/runtime digests, records run metadata, warns and skips isolated missing normalized artifacts during normal mixed wiki/graph updates while keeping `--graph-only` strict, prints the active graph output path after indexing, and exports generated graph markdown; `kb status`, `kb doctor`, and `kb lint` include GraphRAG readiness/freshness checks for settings, input, output tables, vector-store states, and missing or mismatched run metadata; lint also reports manifest raw/normalized artifact drift; `kb ask --method auto|basic|local|global|drift` is the user-facing GraphRAG query controller, checks method-specific readiness, routes the active output directory through GraphRAG's Python query entrypoints, filters unsupported kwargs such as Basic Search `response_type`, consumes returned answers when stdout is empty, refuses blank saves, and preserves raw stdout/stderr separately in saved analysis pages; `kb find` searches direct GraphRAG entity/relationship artifacts plus maintained wiki pages with bounded streamed Parquet scans and globally ranks the merged candidates after dedupe; `kb export` refreshes graph wiki pages with index-run/runtime-digest metadata when complete output exists and clean mode removes only vault markdown absent from the current export set; runtime `.env`, generated input, `output`, `cache`, `logs`, and `graph/runs/*.json` files are ignored. The old `kb graph` command group has been removed.
